...
bestEstimate(AI, B, M, L, S, U) :-
    findall(U1-S1, (
        member(S1, L),
        move(B, S1, M, B2),
        ( AI = 1 -> evaluate_board_lineaire_3inRow(B2, U1) ; evaluate_board(B2, U1) )
    ), Scores),
...
minimax(AI, D, B, M, S, U) :-
    ...
    !,
    (D2 < 5 -> best(AI, D2, B, M, L, S, U) ; bestEstimate(AI, B, M, L, S, U)),  %%% recursively determine the best available move or use bestEstimate
    !
    .
...
best(AI,D,B,M,[S1],S,U) :-
    ...
    (utility(B2, U1), U1 \= 0 ->  %%% if the utility is not 0, use it directly
        ...
        output_value(D,S,U)
    ;
        minimax(AI,D,B2,M2,_S,U),  %%% then recursively search for the utility value of that move.
        S = S1, !,
        output_value(D,S,U),
        !
    ).
...
best(AI,D,B,M,[S1|T],S,U) :-
    move(B,S1,M,B2),             %%% apply the first move (in the list) to the board,
    inverse_mark(M,M2), 
    !,
    (utility(B2, U1), U1 \= 0 ->  %%% if the utility is not 0, use it directly
        best(AI,D,B,M,T,S2,U2),     %%% determine the best move of the remaining moves,
    ...
    ;
        minimax(AI,D,B2,M2,_S,UMinMax),  %%% otherwise, recursively search for the utility value of that move,
        best(AI,D,B,M,T,S2,U2),     %%% determine the best move of the remaining moves,
